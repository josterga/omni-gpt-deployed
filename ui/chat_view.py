...
def render_query_input():
    """Render the chat input - optimized to only trigger on actual input"""
    return st.chat_input("Ask anything about Omni", key="chat_input")

@st.cache_data
def get_assistant_icon():
    """Cache the assistant icon to prevent reloading"""
    return "assets/blobby.png"

def render_message(role: str, message, assistant_icon_path="assets/blobby.png"):
    """Render a single message with proper styling - optimized with containers"""
    if role == "user":
...
def render_unified_sources(unified_results: Dict):
    """Render sources in an expander - optimized with caching for color mapping"""
...
def render_chat_history(messages: List[Dict]):
    """Render the complete chat history - optimized with containers"""
    with st.container():
        for message in messages:
            render_message(message["role"], message["content"])

def render_search_interface():
    """Render the search source selector - optimized to prevent unnecessary reruns"""
    st.markdown("---")
    
    # Search source selector - use session state to prevent unnecessary reruns
    if "search_source" not in st.session_state:
        st.session_state.search_source = "all"
    
    # Only update if user actually changes the selection
    ...
    if search_source != st.session_state.search_source:
